...
| `skills/teams/SKILL.md` | The `/teams` skill itself — **symlinked** into `~/.claude/skills/teams`, not rendered. Edit it here; the live skill is the same file. |
| `bin/teams` | PATH entry point. Resolves its own location through symlinks, so SKILL.md needs no machine-specific paths. |
...
A default run also checks `[4] Global skill install`: the `~/.claude/skills/teams` symlink
points at this repo, no stale `~/.claude/commands/teams.md` shadows it, `teams` resolves on
PATH, and `SKILL.md` is still free of machine-specific paths.
...
- **Never reintroduce `~/.claude/commands/teams.md`.** The skill replaced it; two entries
  named `teams` shadow each other. `install.sh` deletes it, and `verify.sh` asserts it is
  gone.
- **There is no regeneration step.** `~/.claude/skills/teams` is a **symlink** to
  `skills/teams/` in this repo, so editing `SKILL.md` here *is* editing the live skill —
  no sed, no placeholders, no drift. That works because `bin/teams` resolves its own
  location through symlinks, so nothing machine-specific appears in `SKILL.md`. Keep it
  that way: **do not put an absolute path in `SKILL.md`** — it would force rendering back
  and reintroduce the drift class. Re-run `install.sh` only after changing `bin/`, the
  venv, or the repo path. Restart Claude Code to pick up a newly installed skill.
